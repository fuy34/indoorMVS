...
parser.add_argument('--gt_path', default='/data/ScanNet/ScanNet_raw_data/scannet/val_gt/semantic_label_gt/', help='path to gt files')
parser.add_argument('--output_file', default='', help='output file [default: pred_path/semantic_label_evaluation.txt]')
parser.add_argument("--scan_list", default= '../../meta_file/scannet_val_demo.txt', help="which scene(s) to run on")
...
def main():
    ...
    pred_files = [f for f in os.listdir(opt.pred_path) if f.endswith('.txt') and f[:-4] in scenes]#f != 'semantic_label_evaluation.txt' and f!='mesh_eval_sum.txt']
    gt_files = []
    if len(pred_files) == 0:
