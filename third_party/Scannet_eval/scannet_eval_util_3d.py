...
# modified by Fengting /2020/11/01 to python3 gramma
...
import third_party.Scannet_eval.scannet_eval_util as util
...
def export_instance_ids_for_eval(filename, label_ids, instance_ids):
    ...
    with open(filename, 'w') as f:
        for idx, inst_id in enumerate(insts):
            ...
            # print(np.unique(label_ids[loc[0]]))
            label_id, _  = stats.mode(label_ids[loc[0]])
            f.write('%s %d %f\n' % (output_mask_file, label_id, 1.0))
            # write mask
            mask = np.copy(zero_mask)
            mask[loc[0]] = 1
            export_ids(os.path.dirname(filename) +'/' + output_mask_file, mask)
