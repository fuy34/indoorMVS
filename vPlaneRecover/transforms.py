...
# from vPlaneRecover.datasets.rio import load_rio_nyu40_mapping
from vPlaneRecover.datasets.scannet import load_scannet_nyu40_mapping
        ...
        if 'frames' in data:
            for frame in data['frames']:
                if 'instance' in frame:
                    ...
                    break # if one frame does not have, all frames do not have

        # map tsdfs
        # no need to map centroid_prob or plane_norm, they are will aligned alread
        for key in data:
...
def transform_space(data, transform, voxel_dim, origin):
    ...
    for frame in data['frames']:
        frame['pose'] = transform.inverse() @ frame['pose']  # adopt the frame pose to the transformed coordinate system
...
class RandomTransformSpace(object):
    ...
    def __call__(self, data):
        ...
        if self.random_translation:
            t = (torch.rand(3) - 0.5 )/ 4 + .5 # changed by Fengting 01/24/2021 For stability
        else:
            t = .5
        t = t*start + (1-t)*end # it will be centerial crop (out_size < old_size) or centerial padding (out_size > old_size)
...
class FlattenTSDF(object):
    ...
    def __call__(self, data):
        for key in list(data.keys()):
            if key[:3]=='vol':
                tsdf = data.pop(key)
                data['vol_'+key[4:]+'_tsdf'] = tsdf.tsdf_vol.unsqueeze(0) # make 1*nx*ny*nz for future normal compute
