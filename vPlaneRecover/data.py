...
from vPlaneRecover.tsdf import TSDF
import vPlaneRecover.transforms
...
def map_frame(frame, frame_types=[]):
    ...
    if 'depth' in frame_types:
        dpth_pth = frame['file_name_depth']
        # if 'render_depth' in dpth_pth:
        #     dpth_pth = dpth_pth.replace('render_depth', 'depth')
        depth = Image.open(dpth_pth)
...
def map_tsdf(info, data, voxel_types, voxel_sizes):
    ...
    if len(voxel_types)>0:
        for scale in voxel_sizes:
            if 'file_name_vol_%02d'%scale not in info :
                print(info)
            data['vol_%02d'%scale] = TSDF.load(info['file_name_vol_%02d'%scale], voxel_types)
    return data
...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, info_file, transform=None, frame_types=[],
        ...
        self.info = load_info_json(info_file) # dict for info.json we saved in scannet.prepare_scannet_scene()
...
class ScenesDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        data = {'dataset': info['dataset'],
                'scene': info['scene'],
                'instances': info['instances'], # {instance id : label_id} load from aggregation.json
                'frames': frames}
    ...
    def get_frame_ids(self, info):
        ...
        if self.frame_selection=='random':
            ...
            # torch.randint(len(info['frames']), size=[self.num_frames])

            # interval = min(5, len(info['frames']) // self.num_frames)
            # bases  = torch.arange(0, len(info['frames']) - interval, interval)
            # if bases.shape[0] > self.num_frames:
            #     drift = torch.randint(interval, size=[self.num_frames])
            #     perm_id = torch.randperm(bases.shape[0])[:self.num_frames]
            #     idx = bases[perm_id] + drift
            #     return idx
            # else:
...
def collate_fn(data_list):
    ...
    # data_list could be a list of dict, so here make them as a out dict
