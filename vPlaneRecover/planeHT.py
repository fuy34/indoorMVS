...
# The plane detection branch, where three semantic categories (floor, ceiling, wall) are distinguished.
...
class Plane_HT(nn.Module):
    def __init__(self, channel, norm, drop):
        ...
        self.pre_convs =   conv1x1x1(channel, len(SEM_INS_MAP) * channel) #
        self.convs =  nn.ModuleList()
        self.proj = nn.ModuleList()
        self.n_map = len(SEM_INS_MAP)

        for i in range(len(SEM_INS_MAP)):
        ...
        self.top_k_ins = 10
        self.top_k_thres = 0.008 * len(SEM_INS_MAP)
        self.att_base = 1.
