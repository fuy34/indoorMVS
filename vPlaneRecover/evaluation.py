...
def eval_mesh(file_pred, file_trgt, threshold=.05, down_sample=.02):
    ...
    recal = np.mean(recal_mask.astype('float')) #atlas use opposite accuracy definition, they call this precision
    prec_mask = (dist2<threshold)
    precision = np.mean(prec_mask.astype('float')) #recal
    fscore = 2 * precision * recal / (precision + recal)
    metrics = {'comp': np.mean(dist1),
               'accu': np.mean(dist2),
               'recal': recal,#precision,
               'prec': precision,
               'fscore': fscore,
               }
    ...
    pcd_trgt.colors = o3d.utility.Vector3dVector(color)
    ...
    # pcd_pred.colors = o3d.utility.Vector3dVector(np.array([[1., 0, 0]]).repeat(verts_pred.shape[0], axis=0))
    # pcd_pred.colors[prec_mask] = o3d.utility.Vector3dVector( np.array([[0, 1., 0]]).repeat((precision.sum()).astype(np.int), axis=0))

    return metrics, pcd_pred, pcd_trgt
...
def project_to_mesh(from_mesh, to_mesh, attribute, color_mesh=None, dist_thresh=None):
...
def eval_depth(depth_pred, depth_trgt):
    ...
    complete =  np.mean(mask1.astype('float'))
    metrics = {}

    if np.sum(mask1) == 0:
        # only punish complete if no valid pixel exsit
        metrics['AbsRel'] = -1
        metrics['AbsDiff'] = -1
        metrics['SqRel'] = -1
        metrics['RMSE'] = -1
